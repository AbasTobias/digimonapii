...
function obtenerDigimon() {
  fetch(apiUrl)
    ...
    .then(data => {
      const datosLimitados = data.slice(0, 12); 
...
function mostrarDigimon(listaDigimon) {
  ...
  listaDigimon.forEach(digimon => {
    ...
    tarjetaDigimon.classList.add('card', 'digimon-card'); 
...
function Buscar() {
  ...
  fetch(apiUrl)
    .then(response => response.json())
    .then(data => {
      const buscaresultados = data.filter(digimon =>
        digimon.name.toLowerCase().includes(busqueda)
      );
      mostrarDigimon(buscaresultados);
    })
    .catch(error => console.error('Error en la búsqueda, no se ha podido encontrar el digimon:', error));
}
